...
describe('Form', () => {
    ...
    describe('Schema definitions', () => {
        ...
        it('should raise for non-existent definitions referenced', () => {
            ...
            expect(() => (0, test_utils_1.createSchemaForm)({ schema })).toThrow(/#\/definitions\/nonexistent/);
        });
    ...
    describe('Default value handling on clear', () => {
        ...
        it('should not set default when a text field is cleared', () => {
            const { node } = (0, test_utils_1.createSchemaForm)({ schema, formData: 'bar' });
    ...
    describe('Submit button handler', () => {
        it('should not call submit handler before change form state', () => {
            ...
            expect(node.querySelector('.button-submit').disabled).toBeTruthy();
        });
        it('should call submit handler', () => {
            ...
            const onSubmit = global.sandbox.stub();
            const { node } = (0, test_utils_1.createSchemaForm)({
                schema,
                onSubmit,
            });
    ...
    describe('Error contextualization', () => {
        describe('root level', () => {
            it('should denote the error in the field', () => {
                ...
                expect(node.querySelector('.error-item').textContent.trim()).toContain('should NOT have fewer than 8 characters');
            });
        });
        ...
        describe('nested field level', () => {
            it('should denote the error in the field', () => {
                ...
                (0, test_utils_1.fireEvent)(input, 'change');
                expect(node.querySelectorAll('.errors-list')).toHaveLength(1);
                expect(node.querySelector('.error-item').textContent.trim()).toEqual('/level1/level2 should NOT have fewer than 8 characters');
            });
        });
        describe('schema dependencies', () => {
            ...
            it('should only show errors for properties in selected branch', () => {
                ...
                (0, test_utils_1.fireEvent)(field1, 'change');
                const field2 = node.querySelector('#root_field2');
                field2.value = 'not a number';
                (0, test_utils_1.fireEvent)(field2, 'change');
                const liNodes = node.querySelectorAll('.error-item');
                const errors = [].map.call(liNodes, (li) => li.textContent.trim());
            ...
            it('should show errors when branch is other than oneOf', () => {
                ...
                const liNodes = node.querySelectorAll('.error-item');
                const errors = [].map.call(liNodes, (li) => li.textContent.trim());
                expect(errors).toEqual(expect.arrayContaining(['/branch should be equal to one of the allowed values']));
            });
        });
    });
    describe('idSchema updates based on formData', () => {
        ...
        it('should not update id for a falsey value', () => {
            ...
            const { node } = (0, test_utils_1.createSchemaForm)({ schema, formData });
