...
describe('ArrayField', () => {
    describe('Unsupported array schema', () => {
        it('should warn on missing items descriptor', () => {
            const { node } = (0, test_utils_1.createSchemaForm)({ schema: { type: 'array' } });
    ...
    describe('List of inputs', () => {
        ...
        it('should mark a non-null array item as required', () => {
            const { node } = (0, test_utils_1.createSchemaForm)({ schema });
            node.querySelector('.btn-add').click();
            expect(node.querySelector('.field-string input[type=text]').required).toEqual(true);
        });
        ...
        it('should render enough inputs with proper defaults to match ' +
            ...
            const form = (0, test_utils_1.createSchemaForm)({ schema: schemaFormlexSchema, formData: {} });
        ...
        it('should render an input for each default value, even when this is greater than minItems', () => {
            ...
            const { node } = (0, test_utils_1.createSchemaForm)({ schema: schema });
            const inputs = node.querySelectorAll('input[type=text]');
    ...
    describe('Multiple choices list', () => {
        ...
        describe('Checkboxes', () => {
            ...
            it('should render the expected labels', () => {
                const { node } = (0, test_utils_1.createSchemaForm)({ schema });
                const labels = [].map.call(node.querySelectorAll('fieldset span'), (node) => node.textContent.trim());
    ...
    describe('Fixed items lists', () => {
        ...
        it('should fill fields with data', () => {
            const { node } = (0, test_utils_1.createSchemaForm)({ schema, formData: ['foo', 42] });
            const strInput = (node.querySelector('fieldset .field-string input[type=text]'));
            const numInput = (node.querySelector('fieldset .field-number input[type=number]'));
        ...
        it('should handle change events', () => {
            ...
            const numInput = (node.querySelector('fieldset .field-number input[type=number]'));
            strInput.value = 'bar';
            (0, test_utils_1.fireEvent)(strInput, 'change');
            numInput.value = '101';
            (0, test_utils_1.fireEvent)(numInput, 'change');
        ...
        it('should have an add button if additionalItems is an object', () => {
            const { node } = (0, test_utils_1.createSchemaForm)({ schema: schemaAdditional });
        ...
        describe('operations for additional items', () => {
            ...
            it('should add a field when clicking add button', () => {
                ...
                expect(schemaForm.formData).toEqual([1, 2, 'foo', (0, test_utils_1.makeUndefined)()]);
            });
            it('should change the state when changing input value', () => {
                const inputs = node.querySelectorAll('.field-string input[type=text]');
                inputs[0].value = 'bar';
                (0, test_utils_1.fireEvent)(inputs[0], 'change');
                inputs[1].value = 'baz';
                (0, test_utils_1.fireEvent)(inputs[1], 'change');
...
//# sourceMappingURL=array-field-test.js.map