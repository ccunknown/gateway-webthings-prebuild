...
class ArrayField {
    constructor(schema, formData, idSchema, name, definitions, onChange = null, _required = false, disabled = false, readOnly = false) {
    ...
    onAddClick(field) {
        return (event) => {
            ...
            const value = SchemaUtils.getDefaultFormState(itemSchema, 
    ...
    renderArrayFieldItem(field, itemData, index, itemSchema, canRemove) {
      ...
      <div class="array-item array-item-col">
        ...
        const itemIdSchema = SchemaUtils.toIdSchema(itemSchema, itemIdPrefix, this.definitions, itemData);
        const childField = item.querySelector('div.array-item');
        const child = new schema_field_1.default(itemSchema, itemData, itemIdSchema, null, this.definitions, this.onChangeForIndex(index), this.isItemRequired(itemSchema), itemSchema.disabled, itemSchema.readOnly).render();
    ...
    renderArrayFieldset() {
        ...
        field.innerHTML = `${(title ? `<legend id="${`${id}__title`}">${title}</legend>` : '') +
            (description
    ...
    renderNormalArray() {
        ...
        const itemSchema = SchemaUtils.retrieveSchema(schema.items, definitions, items);
    ...
    renderMultiSelect() {
        ...
        const schema = this.schema;
        const definitions = this.definitions;
        const itemsSchema = SchemaUtils.retrieveSchema(schema.items, definitions, items);
    ...
    render() {
        ...
        if (!schema.hasOwnProperty('items')) {
            return new unsupported_field_1.default(schema).render();
        }
