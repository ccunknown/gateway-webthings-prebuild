...
class Database {
    ...
    open() {
        ...
        // circular dependency.
        const filename = path_1.default.join(user_profile_1.default.configDir, 'db.sqlite3');
     ...
     */
    createThing(id, description) {
        return new Promise((resolve, reject) => {
            const db = this.db;
            db.run('INSERT INTO things (id, description) VALUES (?, ?)', [id, JSON.stringify(description)], (error) => {
                if (error) {
                    reject(error);
                }
                else {
     ...
     * @param String description A serialised Thing description.
     */
    updateThing(id, description) {
        return new Promise((resolve, reject) => {
            const db = this.db;
            db.run('UPDATE things SET description=? WHERE id=?', [JSON.stringify(description), id], (error) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    getGroups() {
        return this.all('SELECT id, description FROM groups').then((rows) => {
            const groups = [];
            for (const row of rows) {
                const group = JSON.parse(row.description);
                group.id = row.id;
                groups.push(group);
            }
            return groups;
    ...
    createGroup(id, description) {
        return new Promise((resolve, reject) => {
            const db = this.db;
            db.run('INSERT INTO groups (id, description) VALUES (?, ?)', [id, JSON.stringify(description)], (error) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    updateGroup(id, description) {
        return new Promise((resolve, reject) => {
            const db = this.db;
            db.run('UPDATE groups SET description=? WHERE id=?', [JSON.stringify(description), id], (error) => {
                if (error) {
                    reject(error);
                }
                else {
    ...
    async createUser(user) {
        const result = await this.run('INSERT INTO users ' +
            '(email, password, name, mfaSharedSecret, mfaEnrolled, mfaBackupCodes) ' +
            'VALUES (?, ?, ?, ?, ?, ?)', user.getEmail(), user.getPassword(), user.getName(), user.getMfaSharedSecret(), user.getMfaEnrolled(), JSON.stringify(user.getMfaBackupCodes() || '[]'));
     ...
     */
    async editUser(user) {
        return this.run('UPDATE users SET ' +
            'email=?, password=?, name=?, mfaSharedSecret=?, mfaEnrolled=?, ' +
            'mfaBackupCodes=? WHERE id=?', user.getEmail(), user.getPassword(), user.getName(), user.getMfaSharedSecret(), user.getMfaEnrolled(), JSON.stringify(user.getMfaBackupCodes() || '[]'), user.getId());
    }
    /**
     * Delete a user.
     * @param {Number} userId
    ...
    async createJSONWebToken(token) {
        const result = await this.run('INSERT INTO jsonwebtokens (keyId, user, issuedAt, publicKey, payload) ' +
            'VALUES (?, ?, ?, ?, ?)', token.keyId, token.user, token.issuedAt, token.publicKey, JSON.stringify(token.payload));
        return result.lastID;
    }
    /**
     ...
     */
    createPushSubscription(desc) {
        const description = JSON.stringify(desc);
        const insert = () => {
            return this.run('INSERT INTO pushSubscriptions (subscription) VALUES (?)', description).then((res) => {
                return res.lastID;
            });
        };
